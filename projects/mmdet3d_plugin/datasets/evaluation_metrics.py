...
def evaluation_semantic(pred_occ, confidences, gt_occ, img_metas, class_num, softmax=None):
    results = []
    ...
    for i in range(pred_occ.shape[0]):
        ...
        mask = (gt_i != 255)
        score = np.zeros((class_num+10, 3))
        for j in range(class_num):
            if j == 0: #class 0 for geometry IoU
                score[j][0] += ((gt_i[mask] != 0) * (pred_i[mask] != 0)).sum() #TP
                score[j][1] += (gt_i[mask] != 0).sum() #TP+FN
                score[j][2] += (pred_i[mask] != 0).sum() #TP+FP
            else:
                score[j][0] += ((gt_i[mask] == j) * (pred_i[mask] == j)).sum() #TP
                score[j][1] += (gt_i[mask] == j).sum() #TP+FN
                score[j][2] += (pred_i[mask] == j).sum() #TP+FP

        #Calculating the ECE for current sample
        confidences_i = confidences[i].cpu().numpy()
        ece_score = ECE(10).measure(confidences_i[mask], gt_i[mask]==pred_i[mask])
        score[class_num][0] = ece_score

        if softmax is not None:
            nll = torch.nn.functional.nll_loss(
                input=torch.log(softmax.permute(0,2,3,4,1).squeeze(0)[mask] + 1e-8).cpu(),
                target=torch.from_numpy(gt_i[mask]).long(),
                reduction='mean'
            )
            score[class_num+1][0] = nll.numpy()
