...
def main():
    ...
    train_features, train_labels = get_features_balanced(
        model=model,
    ...
    print("Features consume: " + str((train_features.element_size() * train_features.numel())/1024/1024/1024) + " Gigabyte")

    gmm = gmm_fit(train_features, train_labels, num_classes)
    torch.save(
        obj=gmm,
        f=os.path.join(os.path.dirname(args.checkpoint), f'train_gmm_scale_{feature_scale_lvl}.pt')
    )

    prior_log_prob = get_prior_log_prob(train_labels, num_classes)
    torch.save(
        obj=prior_log_prob,
        f=os.path.join(os.path.dirname(args.checkpoint), f'train_prior_log_prob_scale_{feature_scale_lvl}.pt')
    )
