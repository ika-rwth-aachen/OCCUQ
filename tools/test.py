...
def parse_args():
    ...
    parser.add_argument(
        '--store-dir',
        help='directory to store visualizations if --is_vis is specified.')
    parser.add_argument(
        '--tmpdir',
...
def main():
    ...
    output_dir = f"baseline_{output_dir}"
    if args.front_camera_ood:
        output_dir += f"_front_ood"
    if args.ts_mode is not None:
        output_dir += f"_{args.ts_mode}"
    ...
    if args.front_camera_ood:
        ...
        cfg.model['front_camera_ood'] = True
    
    if args.ts_mode is not None:
        cfg.model['ts_mode'] = args.ts_mode
    ...
    if args.is_vis:
        cfg.model['is_vis'] = True
    cfg.model['store_dir'] = save_dir if args.store_dir is None else args.store_dir
    os.makedirs(cfg.model['store_dir'], exist_ok=True)
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            json_path = os.path.join(cfg.model['store_dir'], 'results.json')
